...
void Spell::SetTargetMap(uint32 effIndex, uint32 targetMode, UnitList& targetUnitMap)
{
    ...
    switch(targetMode)
    {
        ...
        case TARGET_AREAEFFECT_INSTANT:
        {
            ...
            {
                FillAreaTargets(targetUnitMap,m_caster->GetPositionX(), m_caster->GetPositionY(),radius, PUSH_DEST_CENTER, SPELL_TARGETS_AOE);
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        switch(m_spellInfo->Effect[i])
        {
            ...
            {
                if (m_caster->GetTypeId() == TYPEID_PLAYER)
                    if (((Player*)m_caster)->HasMovementFlag(MOVEMENTFLAG_ONTRANSPORT))
    ...
    switch(m_spellInfo->Effect[eff])
    {
        ...
        case SPELL_EFFECT_APPLY_AURA:
            if (m_spellInfo->SpellFamilyName == SPELLFAMILY_GENERIC && m_spellInfo->Category == 0) {
                ...
                if (!m_spellInfo->EffectImplicitTargetA[eff] && !m_spellInfo->EffectImplicitTargetB[eff])
                    return false;
            }
            break;
        default:                                            // normal case
